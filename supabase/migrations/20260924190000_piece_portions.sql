alter table public.meal_items
  add column piece_count float check (piece_count is null or piece_count > 0),
  add column piece_size text check (piece_size in ('small', 'medium', 'large')),
  add constraint meal_items_piece_pair check ((piece_count is null) = (piece_size is null));

alter table public.dish_items
  add column piece_count float check (piece_count is null or piece_count > 0),
  add column piece_size text check (piece_size in ('small', 'medium', 'large')),
  add constraint dish_items_piece_pair check ((piece_count is null) = (piece_size is null));
