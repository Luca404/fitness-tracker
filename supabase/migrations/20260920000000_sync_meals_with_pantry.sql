-- Keep pantry stock in sync with diary entries. The exact pantry row and the
-- amount actually consumed are stored on each meal item so edits and deletes
-- can restore stock before applying their new state.

alter table public.pantry_items
  drop constraint if exists pantry_items_quantity_check,
  drop constraint if exists pantry_items_quantity_check_v2;

alter table public.pantry_items
  add constraint pantry_items_quantity_check check (quantity >= 0);
...
create index meal_items_pantry_item_id_idx on public.meal_items (pantry_item_id)
  where pantry_item_id is not null;

create index dish_items_pantry_item_id_idx on public.dish_items (pantry_item_id)
  where pantry_item_id is not null;
...
begin
  ...
  loop
    ...
    order by
      case when p.id = v_item.pantry_item_id then 0
           when p.food_key = v_item.food_key then 1
           when p.off_food_id = v_item.off_food_id then 2
           else 3 end,
      p.created_at, p.id
    limit 1
    for update;
...
begin
  ...
  with inserted as (
    ...
    select
      ...
      x.fat_g, coalesce(x.source, 'manual'), x.off_food_id
    from jsonb_to_recordset(p_items) as x(
