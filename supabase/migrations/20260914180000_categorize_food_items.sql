-- Persist a stable catalog key and a culinary category across pantry, saved
-- dishes and diary entries. Existing custom data remains available as "other".

alter table public.meal_items
  add column category text not null default 'other',
  add column food_key text,
  add constraint meal_items_category_check check (category in (
    'grain','legume','vegetable','fruit','meat','fish','dairy','egg','fat',
    'sauce','seasoning','sweet','alcohol','beverage','other'
  ));
...
create index meal_items_food_key_idx on public.meal_items (food_key) where food_key is not null;
create index dish_items_food_key_idx on public.dish_items (food_key) where food_key is not null;
create index pantry_items_food_key_idx on public.pantry_items (food_key) where food_key is not null;
...
begin
  ...
  with inserted as (
    ...
    select
      ...
      coalesce(x.category, 'other'), x.food_key, x.calories, x.protein_g,
...
begin
  ...
  with inserted as (
    insert into public.dish_items (
      dish_id, food_name, quantity_g, category, food_key,
      calories, protein_g, carbs_g, fat_g, source, off_food_id
    )
    select
      v_dish.id, x.food_name, x.quantity_g, coalesce(x.category, 'other'), x.food_key,
      x.calories, x.protein_g, x.carbs_g, x.fat_g,
...
begin
  ...
  with inserted as (
    ...
    select
      p_dish_id, x.food_name, x.quantity_g, coalesce(x.category, 'other'), x.food_key,
      x.calories, x.protein_g, x.carbs_g, x.fat_g,
      coalesce(x.source, 'manual'), x.off_food_id
    from jsonb_to_recordset(p_items) as x(
      food_name text, quantity_g float, category text, food_key text,
